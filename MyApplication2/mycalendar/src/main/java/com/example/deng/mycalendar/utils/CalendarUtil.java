...
public class CalendarUtil {
    ...
    private static DateBean initDateBean(int year, int month, int day, int type, Map<String, String> map) {
        ...
        if (map == null) {
            String[] temp =LunarUtil.solarToLunarNum(year,month,day);
            dateBean.setLunar(new String[]{temp[0], temp[1],temp[2],temp[3]});
            temp[0]=LunarUtil.solarToLunarNum(year, month, day)[0];
            temp[1]=LunarUtil.solarToLunarNum(year, month, day)[1];
            temp[2]=LunarUtil.solarToLunarNum(year, month, day)[2];
            temp[3]=LunarUtil.solarToLunarNum(year, month, day)[3];
            temp[4]=LunarUtil.getLunarHoliday(year,month,day);
            dateBean.setLunarHoliday(temp[4]);
        } else {
